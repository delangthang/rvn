...
$user->add_lang('mods/topic_prefix');
...
$uninit = array('post_attachment' => 0, 'poster_id' => $user->data['user_id'], 'enable_magic_url' => 0, 'topic_status' => 0, 'topic_type' => POST_NORMAL, 'post_subject' => '', 'topic_title' => '', 'topic_prefix' => '', 'post_time' => 0, 'post_edit_reason' => '', 'notify_set' => 0);
...
if ($submit || $preview || $refresh)
{
	...
	$post_data['topic_prefix']		= utf8_normalize_nfc(request_var('topic_prefix', '', true));
	...
	if ($mode == 'post' && $post_data['forum_topic_prefix_required'] && empty($post_data['topic_prefix']))
	{
	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			$data = array(
				...
				'topic_prefix'			=> (string) $post_data['topic_prefix'],
				'post_id'				=> (int) $post_id,
				'topic_id'				=> (int) $topic_id,
				'forum_id'				=> (int) $forum_id,
...
$forum_topic_prefixes = explode(';', $post_data['forum_topic_prefixes']);
...
$template->assign_vars(array(
	...
	'TOPIC_PREFIX_SELECT'	=> ($post_data['forum_topic_prefixes']) ? $topic_prefix_select : '',
	...
	'S_TOPIC_POST_ACTION'	=> ($mode == 'post' || ($mode == 'edit' && $post_id == $post_data['topic_first_post_id'])) ? true : false,
